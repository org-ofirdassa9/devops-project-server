...
endpoints_directory = os.path.join(os.path.dirname(__file__), 'app/api/users_service/endpoints')
...
for filename in os.listdir(endpoints_directory):
    ...
    if filename.endswith('.py') and filename != '__init__.py':
        ...
        module = importlib.import_module(f'app.api.users_service.endpoints.{module_name}')

        # Add the router from the imported module
        if hasattr(module, 'router'):
            app.include_router(module.router, prefix=f"/api/users_service/{module_name}", tags=[module_name])
